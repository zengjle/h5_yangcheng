...
const Loader = (function () {
    ...
    _p.load = function (path, type, callback) {
        ...
        loadRes(path, type, function (assets) {
            ...
            for (let i = 0; i < loadingUrl.length; i++)
                loadingUrl[i](assets);
    ...
    _p.addAssets = function (assets) {
        ...
        if (this._loadItems[assets])
            this._loadItems[assets]++;
        else
            this._loadItems[assets] = 1;
    };
    ...
    _p.release = function (assets, isReleaseRelevant) {
        let deps = !isReleaseRelevant ? [getReferenceKey(assets)] : getDependsRecursively(assets);
    ...
    _p.removeAssets = function (assets) {
        ...
        if (this._loadItems[assets]) {
            this._loadItems[assets]--;
            if (this._loadItems[assets] <= 0) {
                release(assets);
                delete this._loadItems[assets];
            }
        } else {
            release(assets);
        }
