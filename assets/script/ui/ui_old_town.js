...
cc.Class({
    ...
    _register_handler: function () {
        ...
        net.on("get_old_town_food_ret", (_msg) => {
            this.shop_info[this.shop_id - 1] = _msg.shop_info;
    ...
    init_shop_info: function (_shop_id) {
        ...
        if (!!this.shop_info[_shop_id - 1]) {
            ...
            _btn_be_get.getComponent(cc.Sprite).spriteFrame = this.spriteFrame_reward_btn[!this.shop_info[_shop_id - 1][0] ? 0 : 1]
    ...
    on_choose_shop: function (_, _shop_id) {
        if (!!this.shop_info[_shop_id - 1][1]) {
        ...
        !this.shop_info[_shop_id - 1][1] && tips.show("暂未解锁该商店!请提升锦鲤等级!");
    },

    on_get_old_town_food: function () {
        net.emit("get_old_town_food", { shop_id: this.shop_id });
    },
    ...
    on_choose_answer: function (_, _answer_id) {
        let answer = data.question.answer[this.question_id - 1];
